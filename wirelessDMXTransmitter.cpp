...
volatile uint8_t dmxBuffer[DMXINPUT_BUFFER_SIZE(START_CHANNEL, NUM_CHANNELS)];
...
#define BEACON_MSG_LEN_MAX NUM_CHANNELS
...
void connect_sta()
{
    ...
    printf("Attempting to connect... AP %s pass %s\n", ap_name, password);

    int ret = 0;
    while( ret = cyw43_arch_wifi_connect_timeout_ms(ap_name, password, CYW43_AUTH_WPA2_MIXED_PSK, 10000)) {
        printf("Ret: %d Failed to connect... AP %s pass %s\n", ret, ap_name, password);
        ledState = !ledState;
        cyw43_gpio_set(&cyw43_state, 0, ledState);
    }
    ...
    cyw43_gpio_set(&cyw43_state, 0, 0);
}
...
int main() {
    ...
    ipaddr_aton(BEACON_TARGET, &send_addr);
    ...
    while(!state->complete) 
    {
        ...
        {
            sleep_ms(100);
            printf("Link Down! ret %d\n", ret);
            cyw43_arch_disable_sta_mode();
            connect_sta();
        }
        #endif

        // check for updated DMX data.  If none, wait and go again
        if(millis() > 100+dmxInput.latest_packet_timestamp()) {
            printf("No DMX data!\n");
            sleep_ms(100);
            dmxLedState = !dmxLedState;
            cyw43_gpio_set(&cyw43_state, 0, dmxLedState);
            continue;
        }
        
        // throttle UDP send rate
        if( millis() > 15 + lastSendTime )
        {
            ...
            req[0] = counter; // slot 0 gets a counter value for debug 
            // copy the dmx data into the packet buffer
            for( uint16_t idx=1; idx <= NUM_CHANNELS; ++idx)
            {
                req[idx] = dmxBuffer[idx];
            }

            // blink the led on dmx transfer
            dmxLedState = !dmxLedState;
            cyw43_gpio_set(&cyw43_state, 0, dmxLedState);

            //snprintf(req, BEACON_MSG_LEN_MAX, "%d\n", counter);
            err_t er = udp_sendto(pcb, p, &send_addr, UDP_PORT);
